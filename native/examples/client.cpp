...
#define SUM 1
#define WEIGHTED_SUM 2
#define EVALUATE_POLYNOMIAL 3
...
void encryptedQueryProcessing(int choice)
{
	// Vector of inputs
	...
	cout << "CLIENT : Parameters are valid: " << boolalpha
		<< context->key_context_data()->qualifiers().parameters_set << endl;
	cout << "CLIENT : Maximal allowed coeff_modulus bit-count for this poly_modulus_degree: "
		<< CoeffModulus::MaxBitCount(poly_modulus_degree) << endl;
	cout << "CLIENT : Current coeff_modulus bit-count: "
	...
	{
		//Stopwatch sw("CLIENT : GaloisKeys creation time");
	...
	Ciphertext ct;
	{
		//Stopwatch sw("CLIENT : Encryption time");
	...
	{
	case SUM:
		ct = sum(dimension, scale, ct, galk);
		break;
	case WEIGHTED_SUM:
		ct = weightedSum(dimension, scale, ct, galk);
		break;
	default:
		break;
	}
	...
	Plaintext pt_result;
	{
		//Stopwatch sw("CLIENT : Decryption time");
	...
	cout << "CLIENT :Result: " << vec_result[0] << endl;

	//Compare the result with actual result
	switch (choice)
	{
	case SUM:
		cout << "CLIENT :True result: " << std::accumulate(inputs.begin(), inputs.end(), decltype(inputs)::value_type(0)) << endl;
		break;
	case WEIGHTED_SUM:
		cout << "CLIENT :True result: " << inner_product(inputs.cbegin(), inputs.cend(), weights.cbegin(), 0.0) << endl;
		break;
	default:
		break;
	}
...
{
    /*
    Evaluating a polynomial function : PI*x^3 + 0.4*x + 1
    ...
    */
    ...
    /*
    multiplication in CKKS causes scales in ciphertexts to grow. 
    The scale of any ciphertext must not get too close
    ...
    */
    ...
    /*
    consider what the scales are at this point. We denote the
...
int main()
{
	...
	{
		cout << "-----------------------------------" << endl;
		cout << "Encrypted Query Processing" << endl;
		cout << "-----------------------------------" << endl;
		cout << "| 1.Sum |" << endl;
		cout << "| 2.Weighted Sum |" << endl;
		...
		{
		case SUM:
			encryptedQueryProcessing(SUM);
			break;
		case WEIGHTED_SUM:
			encryptedQueryProcessing(WEIGHTED_SUM);
			break;
